...
# =============================================================================
# NEW: CUSTOMER SEGMENTATION MODULE (AI Customer Segmentation)
# Added without touching any existing function/logic above.
# =============================================================================

SEGMENT_COLORS = {
    "High-Value Loyal": "#10b981",
    "Growing / Potential": "#3b82f6",
    "At-Risk / Low Engagement": "#ef4444",
}

# Set to True once scaler.pkl (saved from the training notebook) is placed
# alongside kmeans_model.pkl — this makes predictions exactly training-consistent.
USE_SAVED_SCALER = False


@st.cache_resource
def load_segmentation_model(model_path="kmeans_model.pkl"):
    """Loads the trained KMeans model once and caches it across reruns."""
    with open(model_path, "rb") as f:
        return pickle.load(f)


def compute_rfm(df):
    """Builds Recency, Frequency, Monetary (Amount) features from raw
    transactional data — mirrors the training notebook exactly."""
    df = df.copy()
    df.dropna(subset=["CustomerID"], inplace=True)

    # Handles mixed/inconsistent date formats robustly (e.g. "15-12-2010 15:20",
    # "12/1/2010 8:26", ISO strings, etc.) instead of assuming one fixed format.
    ...
    rfm_p["Recency"] = pd.to_timedelta(rfm_p["Diff"]).dt.days

    rfm = pd.merge(rfm_m, rfm_f, on="CustomerID")
    rfm = pd.merge(rfm, rfm_p[["CustomerID", "Recency"]], on="CustomerID")
...
def label_clusters(rfm):
    """Ranks clusters by actual RFM performance in the current batch and
    assigns human-readable segment names (data-driven, not hardcoded)."""
    cluster_means = rfm.groupby("Cluster_Id")[["Amount", "Frequency", "Recency"]].mean()
    score = (
        cluster_means["Amount"].rank()
        + cluster_means["Frequency"].rank()
        - cluster_means["Recency"].rank()
    )
    ordered = score.sort_values(ascending=False).index.tolist()

    names = ["High-Value Loyal", "Growing / Potential", "At-Risk / Low Engagement"]
    label_map = {cid: (names[i] if i < len(names) else f"Segment {cid}") for i, cid in enumerate(ordered)}

    rfm["Segment"] = rfm["Cluster_Id"].map(label_map)
    return rfm, cluster_means


def render_segmentation_module():
    st.title("🎯 AI Customer Segmentation")
    st.caption("RFM-based clustering — upload raw transactional data to auto-segment your customer base.")

    seg_file = st.file_uploader(
        "Upload Transaction Data (must include: InvoiceNo, Quantity, InvoiceDate, UnitPrice, CustomerID)",
        type='csv',
    ...
    if seg_file is None:
        st.markdown("""
            <div style="padding: 20px; border-radius: 12px; border: 1px solid rgba(139, 92, 246, 0.3); background: rgba(15, 23, 42, 0.6); text-align:center;">
                <p style="color:#9ca3af; margin:0;">Upload a transaction-level CSV to generate customer segments.</p>
            </div>
        """, unsafe_allow_html=True)
        return

    try:
        raw = pd.read_csv(seg_file, encoding="ISO-8859-1")
    ...
    try:
        ...
        st.error("⚠️ `kmeans_model.pkl` not found. Place the model file in the app's root directory (next to config.yaml).")
        return
    except Exception as e:
    ...
    if rfm.empty:
        st.warning("No customers left after preprocessing — check your uploaded data.")
        return

    rfm_features = rfm[["Amount", "Frequency", "Recency"]]

    try:
        if USE_SAVED_SCALER:
            with open("scaler.pkl", "rb") as f:
                scaler = pickle.load(f)
            scaled = scaler.transform(rfm_features)
        else:
            scaler = StandardScaler()
            scaled = scaler.fit_transform(rfm_features)

        rfm["Cluster_Id"] = model.predict(scaled)
    except Exception as e:
        st.error(f"Prediction failed: {e}")
        return

    rfm, cluster_means = label_clusters(rfm)

    st.subheader("📌 Segment Overview")
    seg_counts = rfm["Segment"].value_counts()
    cols = st.columns(len(seg_counts))
    for i, (seg, count) in enumerate(seg_counts.items()):
        cols[i].metric(seg, f"{count:,} customers", f"{count/len(rfm)*100:.1f}%")
    ...
    if HAS_PLOTLY:
        ...
        with col1:
            fig = px.pie(
                ...
                color="Segment", color_discrete_map=SEGMENT_COLORS, template="plotly_dark"
            )
            fig.update_layout(paper_bgcolor='rgba(0,0,0,0)', plot_bgcolor='rgba(0,0,0,0)')
            st.plotly_chart(fig, use_container_width=True)
        ...
        with col2:
            fig = px.scatter(
                rfm, x="Recency", y="Amount", color="Segment", size="Frequency",
                title="Recency vs Spend (bubble size = Frequency)",
                color_discrete_map=SEGMENT_COLORS, template="plotly_dark"
            )
            fig.update_layout(paper_bgcolor='rgba(0,0,0,0)', plot_bgcolor='rgba(0,0,0,0)')
            st.plotly_chart(fig, use_container_width=True)
        ...
        with col3:
            fig = px.box(
                rfm, x="Segment", y="Amount", color="Segment",
        ...
        with col4:
            fig = px.bar(
                cluster_means.reset_index(), x="Cluster_Id", y="Frequency",
                title="Avg Purchase Frequency by Cluster",
        ...
        fig, ax = plt.subplots(facecolor='#030712')
        ax.set_facecolor('#0b0f19')
        rfm['Segment'].value_counts().plot(kind='bar', ax=ax, color='#3b82f6')
        ax.set_title("Customer Segment Distribution", color='white')
        st.pyplot(fig)

    st.divider()
    st.subheader("📋 Segmented Customer List")
    st.dataframe(rfm.sort_values("Segment"), use_container_width=True)

    csv_bytes = convert_df_to_csv(rfm)
    st.download_button(
        label="📥 Download Segmented Customers (CSV)",
        data=csv_bytes,
        file_name=f"Customer_Segments_{datetime.date.today()}.csv",
        mime="text/csv",
        key="btn_segments_csv_download",
...
if not authentication_status:
    ...
    # Shown immediately after login. User picks a module, then we route
    # into that module's own workspace (each with its own sidebar nav).
    # -------------------------------------------------------------------
    if 'active_module' not in st.session_state:
        st.session_state.active_module = None

    def go_to_churn():
        st.session_state.active_module = "churn"

    def go_to_segmentation():
        st.session_state.active_module = "segmentation"

    def go_to_landing():
        st.session_state.active_module = None

    if st.session_state.active_module is not None:
        st.sidebar.button("⬅️ Back to Module Selection", on_click=go_to_landing, use_container_width=True)
        st.sidebar.markdown("---")

    if st.session_state.active_module is None:
        st.markdown("""
            <p style="text-align:center; color:#9ca3af; font-size:1.05rem; margin-top: 10px; margin-bottom: 35px;">
                Choose an analytical module to begin workspace configuration.
            </p>
        """, unsafe_allow_html=True)
        ...
        with card_col1:
            st.markdown("""
                <div style="padding: 35px 30px; border-radius: 16px; border: 1px solid rgba(59, 130, 246, 0.3);
                            background: rgba(15, 23, 42, 0.6); text-align:center; min-height: 210px;">
                    <div style="font-size: 2.2rem; margin-bottom: 10px;">📈</div>
                    <h2 style="color: #3b82f6; font-weight: 800; margin: 0 0 12px 0;">Customer Churn Analytics</h2>
                    <p style="color: #9ca3af; font-size: 0.95rem; line-height: 1.5;">
                        Evaluate account loss risk, identify churn drivers, support velocity friction,
                        and predict retention rate loss.
                    </p>
                </div>
            """, unsafe_allow_html=True)
            st.button("Launch Churn Analytics Module", key="btn_launch_churn", on_click=go_to_churn, use_container_width=True)

        with card_col2:
            st.markdown("""
                <div style="padding: 35px 30px; border-radius: 16px; border: 1px solid rgba(16, 185, 129, 0.3);
                            background: rgba(15, 23, 42, 0.6); text-align:center; min-height: 210px;">
                    <div style="font-size: 2.2rem; margin-bottom: 10px;">🎯</div>
                    <h2 style="color: #10b981; font-weight: 800; margin: 0 0 12px 0;">AI Customer Segmentation</h2>
                    <p style="color: #9ca3af; font-size: 0.95rem; line-height: 1.5;">
                        Perform real-time customer clustering using pre-trained ML models
                        (.pkl / .joblib) or standard RFM features.
                    </p>
                </div>
            """, unsafe_allow_html=True)
            st.button("Launch Customer Segmentation Module", key="btn_launch_segmentation", on_click=go_to_segmentation, use_container_width=True)

    elif st.session_state.active_module == "segmentation":
        render_segmentation_module()

    elif st.session_state.active_module == "churn":
        nav_options = [
            "Executive Dashboard", 
        ...
        if uploaded_file is not None:
            ...
            # ---------------------------------------------------------------
            # AI Customer Segmentation does not depend on the churn dataset
            # or its filters — route it here, before the empty-filter check,
            # so it always renders regardless of churn filter results.
            # ---------------------------------------------------------------
            if menu_selection == "🎯 AI Customer Segmentation":
                render_segmentation_module()
